...
async function getStoreSubCategories(paramCatId){
	...
	"/GeneralStore/GetSubCategories?ServerId=" + PRIMARY_SERVER_ID + "&CategoryId=" + paramCatId, {
		headers: {
			"Accept":"application/json",
			"Content-Type":"application/json",
			"X-Zos-Session-Id":gaLOGIN_ARRAY[3]
	...
	gsSUMMARY_REPORT += "TEST: " + gsAPI_SERVER + "/GeneralStore/GetSubCategories?ServerId=" + 
...
//  RETURN ARRAY WITH NINE DIGIT PRODUCT CODES
////////////////////////////////////////////////////////////////
async function getStoreProducts(paramArrayCategories){
	...
	var getStoreProductsResult = await fetch(gsAPI_SERVER + 
		"/GeneralStore/GetCategoryEntries?ServerId=" + PRIMARY_SERVER_ID + "&CategoryId=" + thisCatId, {
		headers: {
			"Accept":"application/json",
			"Content-Type":"application/json",
			"X-Zos-Session-Id":gaLOGIN_ARRAY[3]
	...
	gsSUMMARY_REPORT += "TEST: " + gsAPI_SERVER + "/GeneralStore/GetCategoryEntries?ServerId="
	...
	if (getStoreProductsResult.indexOf("{\"StoreCategoryEntries\":[]}") < 0) {
        ...
        // returning an array with only one prodid and 999999999 = no products found
		if (getStoreProductsResult.indexOf("{\"StoreCategoryEntries\":[]}") > -1) {
			getStoreProductsResult = "";
            getStoreProductsReturn.push(999999999);
		};
	};	
	...
	if (boolAbort == false) {
		...
		let arrayProducts = await getStoreProducts(resultGetStoreCategories);
			if (Array.isArray(arrayProducts)) {
				if ((arrayProducts.length != 0)&&(arrayProducts[0] != 999999999)) {
					boolProductsFound = false;
				};
			};
